...
@section('subhead')
    <title>{{__('admin.w_product_list')}} - {{setting('site_name')}}</title>
@endsection
...
@section('subcontent')
    ...
    <h2 class="intro-y text-lg font-medium mt-10">{{__('admin.w_product_list')}}</h2>
            ...
            <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                <div class="w-56 relative text-gray-700 dark:text-gray-300">
                    <form method="GET">
                        ...
                        <input type="text" class="input w-56 box pr-10 placeholder-theme-13" @if(isset($_GET['name'])) value="{{$_GET['name']}}" @endif name="name" placeholder="{{__('admin.search_by_paper')}}">
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-no-wrap">{{__('admin.point')}}</th>
                            ...
                            <tr class="intro-x row1" data-id="{{ $row->id }}">
                                ...
                                <?php
                                    if(file_exists(public_path()."/upload/e-paper/original/".$row->img) && $row->img!='') {
                                        $url = url('upload/e-paper/original').'/'.$row->img;
                                    }else{
                                ...
                                <td>
                                    <a href="{{$url}}" class="image-popup" title="{{$row->img}}">
                                        <img src="{{$url}}" class="thumb-img-list" alt="{{$row->img}}" onerror="this.onerror=null;this.src='<?php echo url("upload/no-image.png") ?>';">
                                    </a>
                                </td>
                                ...
                                <td class="w-40">
                                    @if($row->status==1)
                                        <a href="{{url('change-status-product/')}}/{{$row->id}}/0">
                                            <div class="flex items-center justify-center text-theme-9">
                                                <i data-feather="check-square" class="w-4 h-4 mr-2"></i> {{__('admin.active')}}
                                            </div>
                                        </a>
                                    @else
                                        <a href="{{url('change-status-product/')}}/{{$row->id}}/1">
                                    ...
                                    <div class="modal" id="delete-confirmation-modal-{{$row->id}}">
                                        <div class="modal__content">
                                            ...
                                            <div class="px-5 pb-8 text-center">
                                                ...
                                                <a href="{{url('delete-product/product')}}/{{$row->id}}" class="button w-24 bg-theme-6 text-white">{{__('admin.delete')}}</a>
                                            </div>
                                        </div>
                                    </div>
                                </td>
