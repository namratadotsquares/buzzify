            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-no-wrap">{{__('admin.time_frame')}}</th>
                        @can('ads-status')
                    ...
                    @if(count($ads))
                        ...
                        @foreach ($ads as $row)
                            <tr class="intro-x row1" data-id="{{ $row->id }}">
                                ...
                                <td>
                                    <img src="{{$row->ad_image}}" class="thumb-img-list" alt="{{$row->title}}" onerror="this.onerror=null;this.src='<?php echo url("upload/no-image.png") ?>';">
                                </td>
                                ...
                                <td>
                                    ...
                                    @else
                                        <div class="flex items-center justify-center text-theme-6">
                                            <i data-feather="check-square" class="w-4 h-4 mr-2"></i>{{__('admin.expired')}}
                                        </div>
                                    @endif
                                </td>
                                @endcan
                                ...
                                <td class="table-report__action w-40">
                                    <div class="flex justify-center items-center">
                                        ...
                                        @can('ads-edit')
                                        <a class="flex items-center mr-3" href="{{url('edit-ad/')}}/{{$layout}}/{{$theme}}/{{$row->id}}" title="{{__('admin.edit')}}">
                                    ...
                                    <div class="modal" id="delete-confirmation-modal-{{$row->id}}">
                                        <div class="modal__content">
                                            ...
                                            <div class="px-5 pb-8 text-center">
                                                ...
                                                <a href="{{url('delete-ad')}}/{{$row->id}}" class="button w-24 bg-theme-6 text-white">{{__('admin.delete')}}</a>
                                            </div>
                                        </div>
                                    </div>
                                </td>
