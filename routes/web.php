<?php

use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Route;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Str;
use Carbon\Carbon;
use App\Models\Blog;
use App\Models\BlogImages;
use App\Models\BlogTranslation;
use App\Helpers;
use App\Models\Category;
use App\Models\SiteContent;
use Image;



/*

|--------------------------------------------------------------------------

| Web Routes

|--------------------------------------------------------------------------

|

| Here is where you can register web routes for your application. These

| routes are loaded by the RouteServiceProvider within a group which

| contains the "web" middleware group. Now create something great!

|

*/

Route::get('/JKLBHSDAKJSHDKIENC', function () {

    Artisan::call('schedule:run');
});


Route::get('/clear', function () {

    Artisan::call('cache:clear');

    Artisan::call('config:clear');

    Artisan::call('view:clear');

    return "Cache is cleared";
});

Route::get('/newsdelet', 'SiteController@newsDeletion');



Route::get('givePermissionToAdmin', 'UserController@givePermissionToAdmin');

Route::get('convertimage', 'BlogController@convertimage');

Route::get('AddCategoryToBlogs', 'BlogController@AddCategoryToBlogs');



/************************

cronjobs

 ***********************/

Route::get('blog/notification/cron-job', 'BlogController@executeCron');
// Finds blogs scheduled within the current minute (minute precision) and returns their IDs.
// Optional: `?publish=1` to publish (status=1) blogs that are still in draft (status=2).
// Optional: `?lookback=5` to include previous minutes (helps avoid missing runs).
Route::get('blog/schedule/cron-job', 'BlogController@scheduledBlogsByMinute')
    ->name('blogScheduleCronJob');

Route::get('deleteImageFromFolders', 'BlogController@deleteImageFromFolders');

Route::get('/buzzify', function () {

    return view('html.app');
})->name('buzzify');



Route::get('quote-notification', function () {

    Artisan::call('quote:cron');

    return "Schedular Called!";
});



Route::get('/', 'SiteController@index');
Route::get('/php_info', function () {
    echo phpinfo();
});

Route::get('/category-blog', 'SiteController@category_blogs');

Route::get('publish/{id}', 'BlogController@publish');

Route::get('/blog-details/{blog_id}', 'SiteController@blog_detail');

// Route::get('/blog-details/{blog_id}', function ($blog_id) {

//     $userAgent = request()->header('User-Agent');
//     // dd($userAgent);
//     $playStore = 'https://play.google.com/store/apps/details?id=com.buzzify';
//     $appStore  = 'https://apps.apple.com/app/buzzify/id6478440886';

//     // // Android → Play Store
//     // if (str_contains($userAgent, 'Android')) {
//     //     return redirect()->away($playStore);
//     // }

//     // iPhone / iPad → App Store
//     if (str_contains($userAgent, 'iPhone') || str_contains($userAgent, 'iPad')) {
//         return redirect()->away($appStore);
//     }else{
//         return redirect()->away($playStore);
//     }

//     // Desktop → optional (redirect Play Store or website)
//     return redirect()->away($playStore);

// });

Route::get('/share-app', function () {

    $userAgent = request()->header('User-Agent');
    // dd($userAgent);
    $playStore = 'https://play.google.com/store/apps/details?id=com.buzzify';
    $appStore  = 'https://apps.apple.com/app/buzzify/id6478440886';

    // // Android → Play Store
    // if (str_contains($userAgent, 'Android')) {
    //     return redirect()->away($playStore);
    // }

    // iPhone / iPad → App Store
    if (str_contains($userAgent, 'iPhone') || str_contains($userAgent, 'iPad') || str_contains($userAgent, 'ipod')) {
        return redirect()->away($appStore);
    }else{
        return redirect()->away($playStore);
    }

    // Desktop → optional (redirect Play Store or website)
    return redirect()->away($playStore);

});

/*

    Admin Routing

*/

Route::middleware(['loggedin', 'Cors'])->group(function () {

    Route::get('admin-login', 'AuthController@loginView')->name('login-view');

    Route::post('admin-login', 'AuthController@login')->name('login');
});



// Route::group(['prefix'=>  config('constant.admin_prefix'), 'middleware' => 'auth'], function() {



Route::middleware('auth')->group(function () {


    Route::resource('roles', 'RoleController');

    Route::get('/delete-role/{id}', 'RoleController@destroy');

    //Route::get('/roles/{layout}/{theme}/{id}', 'RoleController@edit');

    //Route::resource('roles','RoleController');

    Route::get('/admin', 'DashboardController@index');

    Route::get('logout', 'AuthController@logout')->name('logout');

    Route::get('admin_logout', 'AuthController@admin_logout')->name('admin_logout');

    Route::get('page/{layout}/{theme}/{pageName}', 'PageController@loadPage')->name('page');

    // -----------------
    Route::post('/generate-image', 'BlogController@generateImage')->name('reimagine');

    Route::post('/getReimage', 'BlogController@getReimage')->name('getReimage');

    Route::post('/generate-text', 'BlogController@generateText')->name('generateText');
    Route::get('/sendNotificationTest', 'BlogController@sendNotificationTest')->name('sendNotificationTest');

    // ---------------------
    Route::get('/dashboard/{layout}/{theme}', 'DashboardController@index');

    Route::get('/category/{layout}/{theme}', 'CategoryController@index');

    Route::post('/category/{layout}/{theme}', 'CategoryController@index');

    Route::post('/add-update-category', 'CategoryController@addUpdateCategory');

    Route::post('/delete-category', 'CategoryController@deleteCategory');

    //Route::get('/delete-category/{id}', 'CategoryController@deleteCategory');

    Route::get('/change-status-category/{id}/{status}', 'CategoryController@changeCategoryStatus');

    Route::get('/mark-category-featured/{id}/{status}', 'CategoryController@changeCategoryFeatured');

    Route::post('/uploadCategoryThumbImage', 'CategoryController@uploadCategoryThumbImage');

    Route::post('/category-sortable', 'CategoryController@update');

    Route::get('/rss-feed-src/{layout}/{theme}', 'RssFeedController@index');

    Route::post('/rss-feed-src/{layout}/{theme}', 'RssFeedController@index');

    Route::post('/add-update-rss-feed-src', 'RssFeedController@addUpdateRssFeedSrc');

    Route::get('/delete-rss-feed-src/{id}', 'RssFeedController@deleteRssFeed');

    Route::get('/change-status-rss-feed-src/{id}/{status}', 'RssFeedController@changeRssFeedStatus');

    Route::get('/feed-item/{layout}/{theme}', 'RssFeedController@feedItem');

    Route::get('/feed-back/{layout}/{theme}', 'RssFeedController@feedback')->middleware('permission:feedback-list');

    Route::post('/feed-item/{layout}/{theme}', 'RssFeedController@feedItem');

    Route::get('/save-post/{post_id}/{category_id}', 'RssFeedController@saveFeed');

    Route::get('/author/{layout}/{theme}', 'AuthorController@index');

    Route::post('/author/{layout}/{theme}', 'AuthorController@index');

    Route::post('/uploadImage', 'AuthorController@uploadImage');

    Route::post('/addUpdateAuthor', 'AuthorController@addUpdateAuthor');

    Route::get('/change-status-author/{id}/{status}', 'AuthorController@changeAuthorStatus');

    Route::get('/deleteAuthor/{id}', 'AuthorController@deleteAuthor');

    // Constrain layout/theme to avoid clashes with routes like `/blog/{id}/logs`
    $allowedLayouts = '(side-menu|top-menu|simple-menu)';
    $allowedThemes = '(light|dark)';

    Route::get('/blog/{id}/logs', 'BlogController@blogActionLogs')
        ->where('id', '[0-9]+')
        ->middleware('permission:blog-list')
        ->name('blogActionLogs');

    Route::get('/blog/{layout}/{theme}/{id}/analytics', 'BlogController@analytics')
        ->where('layout', $allowedLayouts)
        ->where('theme', $allowedThemes)
        ->where('id', '[0-9]+')
        ->name('analytics');

    Route::get('/blog/{layout}/{theme}', 'BlogController@index')
        ->where('layout', $allowedLayouts)
        ->where('theme', $allowedThemes)
        ->name('blog');

    Route::post('/blog/{layout}/{theme}', 'BlogController@index')
        ->where('layout', $allowedLayouts)
        ->where('theme', $allowedThemes);

    Route::get('/add-blog/{layout}/{theme}', 'BlogController@addBlog');

    Route::get('/edit-blog/{layout}/{theme}/{id}', 'BlogController@editBlog');

    Route::get('/delete-blog/{id}', 'BlogController@deleteBlog');

     Route::post('/delete-multiple-blog', 'BlogController@deleteMultipleBlog');
    Route::post('/bulk-update-blog-schedule', 'BlogController@bulkUpdateSchedule');

    // Blog Visibility Options (admin only)
    Route::get('/blog-visibility-options/{layout}/{theme}', 'BlogVisibilityOptionController@index')->name('blog.visibility.options');
    Route::post('/blog-visibility-options/save', 'BlogVisibilityOptionController@save')->name('blog.visibility.save');
    Route::post('/blog-visibility-options/{id}/toggle', 'BlogVisibilityOptionController@toggleActive')->name('blog.visibility.toggle');
    Route::delete('/blog-visibility-options/{id}', 'BlogVisibilityOptionController@destroy')->name('blog.visibility.destroy');

    Route::get('/deleteBlogImage/{id}', 'BlogController@deleteBlogImage');

    Route::post('/uploadMultipleBannerImage', 'BlogController@uploadMultipleBannerImage');

    Route::post('/uploadAudioFIle', 'BlogController@uploadAudioFIle');

    Route::get('/change-status-blog/{id}/{status}', 'BlogController@changeBlogStatus');

    Route::post('/uploadBlogThumbImage', 'BlogController@uploadBlogThumbImage');

    Route::post('/uploadBannerImage', 'BlogController@uploadBannerImage');

    Route::post('/generate-blog-ai-image', 'BlogController@generateAiImageForBlog');
    Route::post('/save-blog-ai-image', 'BlogController@saveAiImageForBlog');

    Route::post('/addUpdateblogDraft', 'BlogController@addUpdateblogTest');

    Route::post('/addUpdateblog', 'BlogController@addUpdateblog');
    Route::post('/translate-content', 'BlogController@translateContent');

    Route::post('/validateSlug', 'BlogController@validateSlug');

    Route::post('/getFeeds', 'RssFeedController@getFeeds');

    // feedback show the admin dashborad user feedback
    Route::get('/user-feedback/{layout}/{theme}', 'FeedbackController@index');
    Route::post('/delete-multiple-feedback', 'FeedbackController@deleteMultipleFeedback');

    Route::post('/blog-sortable', 'BlogController@update');

    Route::get('/blog-sortable', 'BlogController@update');

    Route::get('/delete-slider-post/{id}', 'BlogController@deleteSliderPost');

    Route::get('/slider/{layout}/{theme}', 'BlogController@slider');

    Route::post('/slider/{layout}/{theme}', 'BlogController@slider');

    Route::get('/users/{layout}/{theme}', 'UserController@index');

    Route::post('/users/{layout}/{theme}', 'UserController@index');

    Route::get('/delete-user/{id}', 'UserController@deleteUser');
    Route::post('/delete-multiple-user', 'UserController@deleteMultipleUser');

    Route::get('/personalization-user/{id}', 'UserController@personalizationUser');

    Route::get('/change-status-user/{id}/{status}', 'UserController@changeUserStatus');

    Route::get('/profile/{layout}/{theme}', 'UserController@profile');

    Route::post('/editProfile', 'UserController@editProfile');

    Route::post('/uploadProfileImage', 'UserController@uploadProfileImage');

    Route::get('/setting/{page}/{layout}/{theme}', 'SiteContentController@index');

    Route::get('/sendMail', 'SiteContentController@sendMail');

    Route::post('/uploadLogoImage', 'SiteContentController@uploadLogoImage');

    Route::post('/uploadLogoFavicon', 'SiteContentController@uploadLogoFavicon');

    Route::post('/uploadBGImage', 'SiteContentController@uploadBGImage');

    Route::post('/updateSetting', 'SiteContentController@updateSetting');

    Route::post('/add-update-social', 'SiteContentController@addUpdateSocial');

    Route::post('/uploadSocialImage', 'SiteContentController@uploadSocialImage');

    Route::get('/delete-social/{id}', 'SiteContentController@deleteSocial');

    Route::get('/change-status-social/{id}/{status}', 'SiteContentController@changeSocialStatus');

    Route::post('/settingPermission', 'SiteContentController@settingPermission');

    Route::get('/search-log/{layout}/{theme}', 'SearchController@index');

    Route::post('/search-log/{layout}/{theme}', 'SearchController@index');
    Route::get('/delete-search-log/{id}', 'SearchController@deleteSearchLog');
    Route::post('/delete-multiple-search-log', 'SearchController@deleteMultipleSearchLog');

    Route::get('/cms-pages/{layout?}/{theme?}', 'CmsPagesController@index');

    Route::post('/cms-pages/{layout}/{theme}', 'CmsPagesController@index');

    Route::get('/add-cms-page/{layout}/{theme}', 'CmsPagesController@addPage')->name('addcms');

    Route::post('/add-cms-page/{layout?}/{theme?}', 'CmsPagesController@store')->name('store-cms');

    Route::post('/edit-cms-page/{layout}/{theme}/{id}', 'CmsPagesController@editPage');

    Route::get('/edit-cms-page/{layout}/{theme}/{id}', 'CmsPagesController@editPage');

    Route::get('/delete-cms/{id}', 'CmsPagesController@deletePage');

    Route::post('/uploadCMSBannerImage', 'CmsPagesController@uploadCMSBannerImage');

    Route::post('/addUpdateCMSPage', 'CmsPagesController@addUpdateCMSPage');

    Route::get('/sub-admin/{layout}/{theme}', 'SubAdminController@index');

    Route::post('/sub-admin/{layout}/{theme}', 'SubAdminController@index');

    Route::post('/add-update-sub-admin', 'SubAdminController@addUpdateSubAdmin');

    Route::get('/delete-sub-admin/{id}', 'SubAdminController@deleteSubAdmin');

    Route::get('/change-status-sub-admin/{id}/{status}', 'SubAdminController@changeSubAdminStatus');

    Route::post('/uploadSubAdminThumbImage', 'SubAdminController@uploadSubAdminThumbImage');

    Route::get('/setpermissions', 'SubAdminController@setpermissions');

    Route::get('/live-news/{layout}/{theme}', 'LiveNewsController@index');

    Route::post('/live-news/{layout}/{theme}', 'LiveNewsController@index');

    Route::post('/add-update-live-news', 'LiveNewsController@addUpdateNews');

    Route::post('/upload-logo', 'LiveNewsController@uploadLogo');

    Route::get('/delete-live-news/{id}', 'LiveNewsController@deleteNews');

    Route::get('/change-status-live-news/{id}/{status}', 'LiveNewsController@changeNewsStatus');

    Route::get('/e-paper/{layout}/{theme}', 'EpaperController@index');

    Route::get('/wallet-product/{layout}/{theme}', 'WalletController@index');

    Route::post('/add-update-product', 'WalletController@addUpdate');

    Route::get('/request-product/{layout}/{theme}', 'WalletController@index');

    Route::post('/req-update-product', 'WalletController@reqUpdate');

    Route::get('/letest-request-product/{layout}/{theme}', 'WalletController@letest_list');

    Route::get('/change-status-product/{id}/{status}', 'WalletController@changeStatus');

    Route::get('/change-status-story/{id}/{status}/{user_id}', 'WalletController@changestoryStatus');

    Route::get('/delete-product/{id}', 'WalletController@deleteEpaper');
      Route::get('/delete-product/product/{id}', 'WalletController@deleteProduct');

    Route::get('/story/{layout}/{theme}', 'WalletController@story')->name('list_stories');

    Route::get('/letest-story/{layout}/{theme}', 'WalletController@storyletest');

    Route::get('/viewstory/{id}/{layout}/{theme}', 'WalletController@view')->name('viewstory');

    Route::post('/e-paper/{layout}/{theme}', 'EpaperController@index');

    Route::post('/add-update-e-paper', 'EpaperController@addUpdateEpaper');

    Route::post('/upload-logo-e-paper', 'EpaperController@uploadLogo');

    Route::get('/delete-e-paper/{id}', 'EpaperController@deleteEpaper');
    

    Route::get('/change-status-e-paper/{id}/{status}', 'EpaperController@changeEpaperStatus');

    Route::post('/uploadPdf', 'EpaperController@uploadPdf');

    Route::get('/news-api-post/{layout}/{theme}', 'NewsApiController@index');
    Route::post('/news-api-post/{layout}/{theme}', 'NewsApiControllers@index');
    Route::get('/save-news-api-post', 'NewsApiController@saveNewsApiPost');
    Route::post('/save-news-api-post', 'NewsApiController@saveNewsApiPost');

    // Event Registry (NewsAPI.ai) Routes
    Route::get('/fetch-event-registry-news', 'NewsApiController@fetchEventRegistryNews')->name('eventRegistry.fetchNews');
    Route::get('/fetch-event-registry-filters', 'NewsApiController@fetchEventRegistryFilters')->name('eventRegistry.fetchFilters');



    // translation routes

    Route::post('/get-category-translation/', 'CategoryTranslationController@show');

    Route::post('/translate-category', 'CategoryTranslationController@store');

    Route::post('/get-live-news-translation/', 'LiveNewsTranslationController@show');

    Route::post('/translate-live-news', 'LiveNewsTranslationController@store');

    Route::post('/get-e-paper-translation/', 'EpaperTranslationController@show');

    Route::post('/translate-e-paper', 'EpaperTranslationController@store');

    Route::get('/edit-cms-page-translation/{layout}/{theme}/{id}', 'CmsPagesTranslationController@index');

    Route::post('/get-cms-page-translation/', 'CmsPagesTranslationController@show');

    Route::post('/translate-cms-page', 'CmsPagesTranslationController@store');

    Route::get('/edit-blog-translation/{layout}/{theme}/{id}', 'BlogTranslationController@index');

    Route::post('/get-blog-translation/', 'BlogTranslationController@show');

    Route::post('/translate-blog/', 'BlogTranslationController@store');

    Route::get('/send-notification/{layout}/{theme}', 'SendPushNotifictionController@index');

    Route::post('/send-notification', 'SendPushNotifictionController@sendNotification');

    Route::get('/send-blog-notification/{blog_id}/', 'BlogController@sendBlogNotification');

    Route::get('/quotes/{layout}/{theme}', 'QuoteController@index');

    Route::post('/quotes/{layout}/{theme}', 'QuoteController@index');

    Route::post('/get-quote-translation/', 'QuoteTranslationController@show');

    Route::post('/translate-quote', 'QuoteTranslationController@store');

    Route::post('/add-update-quote', 'QuoteController@store');

    Route::get('/change-status-quote/{id}/{status}', 'QuoteController@changequoteStatus');

    Route::get('/delete-quote/{id}', 'QuoteController@deletequote');

    Route::get('/autocomplete', 'SendPushNotifictionController@postAutocomplete');

    Route::post('/autocomplete', 'SendPushNotifictionController@postAutocomplete');

    Route::post('/uploadLiveNewsLogo', 'SiteContentController@uploadLiveNewsLogo');

    Route::post('/uploadEpaperLogo', 'SiteContentController@uploadEpaperLogo');
});





/***************** Site routes *****************/

Route::get('user-login', 'SiteAuthController@loginView')->name('login-view');

Route::post('user-login', 'SiteAuthController@login')->name('login');

Route::get('user-signup', 'SiteAuthController@signupView')->name('signup-view');

Route::post('user-signup', 'SiteAuthController@signup')->name('signup');

Route::get('forget-password', 'SiteAuthController@forgetView')->name('forget-password-view');

Route::post('forget-password', 'SiteAuthController@forget_password')->name('forget-password');

Route::post('reset-password', 'SiteAuthController@reset_password')->name('reset-password');

Route::get('/edit-profile', 'EditProfileController@index');

Route::post('edit-profile', 'EditProfileController@edit_profile')->name('edit-profile');

Route::get('/saved-stories', 'EditProfileController@saved_stories');

Route::get('/my-stories', 'EditProfileController@my_stories')->name('my_stories');

Route::get('/add-my-stories', 'EditProfileController@add_my_stories')->name('add_my_stories');

Route::post('/add-my-stories', 'EditProfileController@save_my_stories')->name('save_my_stories');

Route::get('/view_my_story/{id}', 'EditProfileController@view_my_story')->name('view_my_stories');

Route::get('/feedback', 'EditProfileController@my_feedback');

Route::post('/feedback', 'EditProfileController@save_feedback')->name('my_feedback');

Route::get('/manage-request', 'EditProfileController@manage_request');

Route::get('/redeem-request', 'EditProfileController@redeem_request');

Route::get('/redeem-request/{product_id}', 'EditProfileController@redemm_points')->name('redemm_points');

Route::get('/delete-story/{id}', 'EditProfileController@deleteStory');

Route::post('bookmark', 'EditProfileController@bookmark')->name('bookmark');

Route::post('upload-profile-image', 'EditProfileController@uploadProfileImage');

Route::get('delete-account', 'EditProfileController@deleteAccount');

Route::get('logout', 'SiteAuthController@logout')->name('logout');

/*************************Site routes end***************** */



/*

CMS Pages

*/

Route::get('/{page_name}', 'SiteController@cms'); // All CMS Pages dynamic

Route::post('/set-language/', 'SiteController@setLangugae');

//Route::get('/blog-details/{category}/{blog_id}', 'SiteController@blog_detail');

// Route::get('/blog-details/{slug}', 'SiteController@blog_detail');

Route::post('load-blog', 'SiteController@loadDataAjax');

Route::post('get-blog-speech', 'SiteController@blogSpeech');

//ads-module

Route::get('/ads/{layout?}/{theme?}', 'AddController@index');
Route::post('/ads/{layout?}/{theme?}', 'AddController@add');
Route::get('/add-ads-slider/{id}', 'AddController@addImages')->name('add-slider');
Route::get('/create-add/{layout}/{theme}', 'AddController@create');
Route::get('/edit-ad/{layout}/{theme}/{id}', 'AddController@editAd');
Route::post('/edit-ad/{layout}/{theme}/{id}', 'AddController@update');
Route::get('/delete-ad/{id}', 'AddController@deleteAd');
Route::get('/deleteAdImage/{id}', 'AddController@deleteAddImage');
Route::get('/change-status-ad/{id}/{status}', 'AddController@changeAdStatus');
Route::get('/ads/{id}/analytics/{layout}/{theme}', 'AddController@analytics')->name('analytics-ads');
Route::post('/ads-sortable', 'AddController@sortAds');
Route::post('/delete-ad-image', 'AddController@deleteAdImage');
Route::get('/change-order/{layout}/{theme}/{id}', 'AddController@changeOrder');
Route::get('/show-preview/{layout}/{theme}/{id}', 'AddController@showPreview');
Route::post('/ads-media-sortable', 'AddController@updatePosition');
Route::post('/addUpdateRedirectedUrl', 'AddController@addUpdateRedirectedUrl');


// news-ad-module

Route::get('/news-ads/{layout?}/{theme?}', 'AdsController@index');
Route::post('/news-ads/{layout?}/{theme?}', 'AdsController@add');
Route::get('/news-create-ads/{layout}/{theme}', 'AdsController@create');
Route::get('/changeNews-status-ad/{id}/{status}', 'AdsController@changeNewsAdStatus');
Route::get('/delete-newsad/{id}', 'AdsController@deleteNewsAd');
Route::get('/edit-Newsad/{layout}/{theme}/{id}', 'AdsController@editAd');
Route::post('/edit-Newsad/{layout}/{theme}/{id}', 'AdsController@update');



Route::get('/cron/run-cron-news', 'NewsApiController@runCronNews');
Route::get('/cron/remove-featured', function () {
    // Remove 'featured' flags from blogs and categories after 24 hours
    $dateLimit = Carbon::now()->subHours(24);

    // Unset featured flag on blogs that were marked featured more than 24 hours ago

   $blogs = Blog::where('is_featured', 1)->where('schedule_date', '<', $dateLimit)->get();
    
    foreach ($blogs as $blog) {
        $blog->is_featured = 0;
        $blog->is_slider = 0;
        $blog->save();
    }


    return "Remove featured cron executed successfully.";
});
Route::get('/cron/news-deletion', function () {
    $newsdelet = SiteContent::find(64);
    $deleted_days_limit = (int)$newsdelet->value;
    $dateBeforeSixDays = Carbon::now()->subDays($deleted_days_limit)->toDateString();


    $blog = Blog::where('schedule_date', '<', $dateBeforeSixDays)->where('status', 1)->get();

    foreach ($blog as $blogs) {
        $blogImage = BlogImages::where('blog_id', $blogs->id)->get();
        foreach ($blogImage as $blogImages) {
            $imageUrl = 'upload/blog/banner/360/' . $blogImages->image;
            $fullImagePath = public_path($imageUrl);

            if (file_exists($fullImagePath)) {
                unlink($fullImagePath);
            }
            $deleteImage = BlogImages::find($blogImages->id);
            $deleteImage->delete();
        }
        $deleteblog = Blog::find($blogs->id);
        $deleteblog->delete();
         return "News deletion cron job executed successfully.";
    }
    return "News deletion cron job executed successfully.";
});
