...
class Ads extends Model
{
    ...
    protected $table = "ads";
       protected $fillable = ['title','frequency','order','start_date','end_date','view','click','status'];
    use SoftDeletes;
    ...
    public static function getAllAds($search = ''){
        try {
            ...
            if(isset($search['name']) && !empty($search['name'] && $search['name'] != '')){
              $contact = $contact->where(DB::raw('LOWER(title)'), 'like', '%'.strtolower($search['name']). '%');
            }

            if(isset($search['start_date']) && !empty($search['start_date'] && $search['start_date'] != '')){
                $contact = $contact->where('start_date',">=",$search['start_date']);
            }
            if(isset($search['end_date']) && !empty($search['end_date'] && $search['end_date'] != '')){
                $contact = $contact->where('end_date',"<=",$search['end_date']);
            }
            ...
            if(count($data)){
                foreach($data as $row){
                    $ads_image = Ads_images::where('ad_id',$row->id)->orderBy('position','DESC')->first();
                    if($ads_image){
                        $row->ad_image = url('storage/ads/'.$ads_image->file);
                    }
                }
            }
            return $data;
